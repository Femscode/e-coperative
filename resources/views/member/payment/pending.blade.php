    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Pending Dues</h4>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Available Payable Dues</h4>
                </div><!-- end card header -->
                                            ...
                                            <tbody>
                                                @foreach ($months as $month)
                                                    <tr>
                                                        ...
                                                        <input type="hidden" @isset($month['amount']) value="{{ $month['amount'] }}" @else value="{{ $plan->getMondays($month['month']) * $plan->monthly_dues + $plan->monthly_charge }}" @endisset name="fee[]">
                                                        <th scope="row">
                                                            <div class="form-check">
                                                                <input class="form-check-input controlledCheckbox" @isset($month['amount']) data-id="{{ $month['amount'] }}" @else data-id="{{ $plan->getMondays($month['month']) * $plan->monthly_dues + $plan->monthly_charge }}" @endisset name="check[]" type="checkbox" id="inlineCheckbox2"  >
                                                            </div>
                                                        </th>
                                                        ...
                                                        <td> <input type="hidden" name="original[]"  @isset($month['amount']) value="{{ $month['amount'] }}" @else value="{{ $plan->getMondays($month['month']) * $plan->monthly_dues  }}" @endisset> @isset($month['amount']) {{ number_format($month['amount'] , 2)}} @else {{ number_format($plan->getMondays($month['month']) * $plan->monthly_dues + $plan->monthly_charge, 2)}} @endisset </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
