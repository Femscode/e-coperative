    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                ...
                <div class='alert alert-primary' style='border:1.5px dashed secondary'>
                    <input value='manual' style='transform: scale(2); cursor:pointer' class='tracking_type m-2 p-2'
                        name='delivery_type' type='radio' @if($user->tracking_type == 'manual') checked @endif> Manual
                    <input value='automatic' style='transform: scale(2); cursor:pointer' class='tracking_type m-2 p-2'
                        name='delivery_type' type='radio' @if($user->tracking_type == 'automatic') checked @endif >
                    Automatic
                </div>
            </div>
            <div class='row'>
                <div class='col-md-6'>
                    <div class=" alert alert-primary" role="alert">
                        You are currently running on an automatic tracking which requires that your
                        delivery process needs to be completed within <strong>{{ $user->tracking_time }} Minutes.</strong>
                                ...
                                <div class="card-body">
                                    <div class="d-flex">
                                        <div class="flex-grow-1">
                                            ...
                                            <h4 class="mb-0" id='completed_delivery'>{{ count($completed_deliveries) }}
                                            </h4>
                                        </div>
            ...
            <div class='col-md-4'>
                ...
                <div class="card card-xl-stretch mb-xl-8">
                    ...
                    <div class="p-4 border-0">
                        <div class='row'>
                            <div class='col-md-6'>
                                <h3 class="card-title fw-bolder text-dark">Delivery Time</h3>
                            </div>
                    ...
                    <div class="card-body pt-2">
                        <!--begin::Item-->
                        <form id='tracking_time_form'>
                            <div class="input-group">
                                <label class='text-muted'>Time taken for delivery process to be completed after payment
                                    confirmation.</label>
                                <input id='tracking_time' type="number" class="form-control"
                                    value='{{ $user->tracking_time }}'>
                            ...
                            <div class="table-responsive dataTables_length" id='datatable_length'>
                                <table class="mytable table table-bordered w-100 align-middle mb-0">
                                    ...
                                    <tbody id='myTbody'>
                                        ...
                                        <tr id='tr_{{ $delivery->id }}' @if($delivery->status == 0)
                                            style='background:#cbffda' @else style='background:#ffcccb' @endif>
                                            ...
                                            <td id='rider_name_{{ $delivery->id }}'>{{ $delivery->rider_name ??
                                                "pending" }}</td>
                                            ...
                                            <td id='pick_up_time_{{ $delivery->id }}'>{{ date('h:i A',
                                                strtotime($delivery->pick_up_time)) ?? 'pending' }}</td>


                                            <td>
                                                <div class="d-flex justify-content-end flex-shrink-0">
                                                    @if($delivery->status == 2)
                                                    <a id='change_status_{{ $delivery->id }}'
                                                        data-change='{{ $delivery->id }}' style='margin-right:5px'
                                                        data-id='{{ $delivery->uuid }}'
                                                        class='confirm_payment btn btn-sm btn-secondary mr-2'>
                                                        Deny Payment
                                                    </a>
                                                    ...
                                                    <a id='change_status_{{ $delivery->id }}'
                                                        data-change='{{ $delivery->id }}' style='margin-right:5px'
                                                        data-id='{{ $delivery->uuid }}'
                                                        class='confirm_payment btn btn-sm btn-info mr-2'>
                                                        Confirm Payment
                                                    </a>
                                                    @else
                                                    @endif
                                                    ...
                                                    <a data-id='{{ $delivery->uuid }}'
                                                        class="mark_complete btn btn-dark btn-sm" style='margin-right:4px'>
                                                        Mark Complete
                                                    </a>
                                                    @if($delivery && $delivery->rejected == 0)
                                                    <a id='change_status_{{ $delivery->id }}'
                                                        data-change='{{ $delivery->id }}' style='margin-right:5px'
                                                        data-id='{{ $delivery->uuid }}'
                                                        class='cancel_order btn btn-sm btn-danger text-white mr-2'>
                                                        Cancel Order
                                                    </a>
                                                    @else
                                                    <a id='change_status_{{ $delivery->id }}'
                                                        data-change='{{ $delivery->id }}' style='margin-right:5px'
                                                        data-id='{{ $delivery->uuid }}'
                                                        class='cancel_order btn btn-sm btn-info text-white mr-2'>
                                                        Approve Order
                                                    </a>
...
<script>
        $(document).ready(function() {
            ...
            $('body').on('click', '.cancel_order', function() {
                ...
                var el = this;
                // alert(user_id);
                cancelPayment(el, id, change_id);
            });


            async function cancelPayment(el, id, change_id) {

                const willUpdate = await Swal.fire({
                    title: "Cancel This Order",
                    text: `Cancel order only when payment is not confirmed, or there is an issue with delivery!`,
                    icon: "warning",
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "Yes!",
                    showCancelButton: true,
                ...
                if (willUpdate.isDismissed == false) {
                    //performReset()
                    performCancelPayment(el, id, change_id);
                }
            }


            function performCancelPayment(el, id, change_id) {

                try {
                    $.get('{{ route("cancel_order") }}?id=' + id,
                        function(data, status) {
                            console.log(status);
                            console.table(data);
                            if (status === "success") {
                    ...
                    success: function(data) {
                        ...
                        if (data[0] == true) {
                            ...
                            $(".check_order:checked").each(function() {
                                ...
                                $("#pick_up_time_" + id).text((new Date(data[1].pick_up_time)).toLocaleTimeString([], {
                                    hour: '2-digit',
                                    minute: '2-digit'
                                }));
            ...
            $("#tracking_time_form").on("submit", async function(e) {
                Swal.fire('Updating, please wait...')
                e.preventDefault();
                var fd = new FormData;
                fd.append('tracking_time', $("#tracking_time").val());
                $.ajax({
                    type: 'POST',
                    url: "{{route('update_tracking_time')}}",
                    data: fd,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(data) {
                        ...
                        Toast.fire({
                            icon: 'success',
                            title: 'Delivery tracking time updated'
                        })
                    ...
                    error: function(data) {
                        console.log(data)
                        Swal.close()
                        Swal.fire('Opps!', 'The delivery time must be of a minimum of 10 Minutes', 'error')
                    }
                })
