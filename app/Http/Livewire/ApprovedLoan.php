...
class ApprovedLoan extends Component
{
    ...
    public function render()
    {
        ...
        if($this->search == ''){
            $data['loans'] = MemberLoan::where('company_id',$company->uuid)->where('approval_status', 1)->where('payment_status', 0)->where('status', "Awaiting")->paginate(10);
        }else{
            $data['loans'] = MemberLoan::where('company_id',$company->uuid)->where('approval_status', 1)->where('payment_status', 0)->where('status', "Awaiting")->where(function ($query) {
